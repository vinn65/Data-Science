...
def p_if_statement(p):
    ...
    if len(p) == 6:
        ...
        quadruples.append(("IF_END", None, None, None))
    if len(p) == 4:
        quadruples.append(("IF_FALSE", p[2], None, None))
        for statement in p[4]:
            quadruples.append(statement)
        quadruples.append(("IF_END", None, None, None))
    return ("IF_ELSE_STATEMENT", p[2], p[4], p[7] if len(p) == 8 else [])
...
def p_binary_expression(p):
    ...
    p[0] = ("BINARY_EXPRESSION", p[2], p[1], p[3], temp)
...
# Define the main function to interpret the code
def run_code(code):
    global quadruples
    quadruples = []
    result = parser.parse(code, lexer=lexer)
    for quadruple in quadruples:
        operator, operand1, operand2, result = quadruple
        if operator == "=":
            symbol_table[result] = operand1
        elif operator == "+":
            symbol_table[result] = operand1 + operand2
        elif operator == "-":
            symbol_table[result] = operand1 - operand2
        elif operator == "*":
            symbol_table[result] = operand1 * operand2
            ...
            if not operand1:
                current_quadruple = result
                while current_quadruple is not None:
                    operator, operand1, operand2, result = current_quadruple
                    if operator == "GOTO":
                        current_quadruple = result
                    elif operator == "LABEL":
                        current_quadruple = None
                    elif operator == "PRINT":
                        print(operand1)
                        current_quadruple += 1
                    else:
